export default async function backup(db) {

            try {

                console.log("Création du fichier de sauvegarde SERVERS");

                const servers = await db._get_servers_table();
                ...
                await db.erase(JSON.stringify(backup), "./data/backup_servers_latest.json");

                console.log("Création du fichier de sauvegarde WARNS");

                const warns = await db._get_warns_table();
                ...
                await db.erase(JSON.stringify(backup), "./data/backup_warns_latest.json");


                console.log("Création du fichier de sauvegarde LINKASSASSIN");

                const linkAssassin = await db._get_linkassassin_table();
